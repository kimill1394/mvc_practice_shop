<?php
  abstract class AppBase {
    ...
    public function run() {
      try {
        $parameters = $this->_router->getRouteParams($this->_request->getPath()); // getPath()는
        // 정의되어 있는 라우팅 정보와 리퀘스트 url을 참고하여 경로 정보를 매칭시켜 반환, 정의되어 있지 않는 경우 false 반환
        if($parameters === false) throw new FileNotFindException('NO ROUTE '.$this->_request->getPath());
        // 경로정보가 없으면 에러 발생!
    ...
    protected function getcontent($controllerName, $action, $parameters=array()) {
      ...
      if($controller === false) throw new FileNotFindException($controllerClass.' NOT FOUND.');
